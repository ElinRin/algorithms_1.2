...
void hirschberg_alg(const string & line1, const string &line2, int first_1, int last_1, int first_2, int last_2, int &result)
{
        if (first_2 == last_2)
        {
        ...
        if (first_1 == last_1 - 1)
		{
            ...
            {
            	result++;
            	if ((line2[i] == line1[first_1]) & label)
            	{
            		result--;
            		label = false;
            	} 
...
int main()
{
  ...
  wagner_fischer_alg(l1,l2,res);
	cout << res << '\n';
	res = 0;
    hirschberg_alg(l1,l2, 0, l1.length(), 0, l2.length(), res);
	cout << res << '\n';
